...
class GSMVoiceCall_ModemUrcHandler : public ModemUrcHandler 
{
  public:
    ...
    void handleUrc(const String& urc)
    {
      ...
      // if (urc.startsWith("RING")) 
      // {
      //   _callStatus = RECEIVINGCALL;
        
      //   GSM_LOGDEBUG(F("GSMVoiceCall::handleUrc: RECEIVINGCALL"));
      // } 
      // else if(urc.startsWith("+CLIP: "))
      // {

      //   _dtmfBuffer = urc.substring(String("+CLIP: ").length() + 1);
      // }
      // else if(urc.startsWith("NO CARRIER")) 
      // {
      ...
      if(urc.startsWith("+CLCC: "))
      {
        char status = urc[String("+CLCC: 0,0,").length()];
        switch(status)
        {
          case '0':
            _callStatus = TALKING;
